/**
 * plugins/seasonal-synergy/services/sessionService.js
 *
 * Tracks active leaderboard screenshot upload sessions.
 * Prevents multiple moderators from conflicting and organizes multi-image uploads.
 */
...
function _cleanup() {
  ...
  for (const [userId, session] of sessions.entries()) {
    if (now > session.expires) {
      sessions.delete(userId);
      logger.info("SeasonalSynergy", `Expired session for user ${userId} cleaned up.`);
    }
  }
}
