...
module.exports = async function emojiBrowserButtons(interaction) {
  ...
  if (interaction.customId === "admin_select_action") {
      ...
      if (actionType === "delete") {
          ...
          // Note: BrowserUI will automatically refresh if user interacts with it again,
          // but for instant feedback we just edit the ephemeral. 
          // Advanced: tracking the message ID to force refresh the UI message embed 
          // is complicated across restarts. For now, inline ephemeral response is safest.
      }
      return;
  }
